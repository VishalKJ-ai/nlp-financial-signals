...
The pipeline fixes it. It reads **92 of the 93 Fed press conferences since 2011** (one fell to a URL anomaly on the Fed's site — see the data-quality notes below), uses transformer language models to discover *what* the Chair is talking about (unsupervised — no labels, no keyword lists) and *how* they talk about each theme, and outputs a **per-theme sentiment time series** instead of one washed-out score. The theme-level signals explain roughly **20× more in-sample variance** in the market's meeting-day reaction than the document-level score they replace — though see the honest caveat below: that raw ratio does not survive a degrees-of-freedom penalty.
...
| Does it agree with the established benchmark? | Meeting-level r = 0.63 with the Loughran-McDonald dictionary — passes the pre-specified benchmark **consistency** test (convergent validity with an accepted instrument, not proof of correct measurement) |
| Does it carry market-relevant information? | Partly — theme-level R² ≈ 0.127 vs 0.006 for the document baseline on meeting-day 2-year yield moves, and household-sector themes are individually significant. **Honest caveats:** that is an in-sample comparison of a 10-regressor model against a 1-regressor baseline on n = 92, and the advantage largely disappears once penalised — adjusted R² 0.019 vs −0.005, joint F-test p = 0.419, and on the futures target the adjusted figure is *negative* (−0.043). Nothing survives multiple-testing correction. No out-of-sample test was run. |
